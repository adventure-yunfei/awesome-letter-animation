...
/***/ function(module, exports, __webpack_require__) {
	...
	var animejs = __webpack_require__(2);
	var letter_1 = __webpack_require__(3);
	function awesomeLetterAnimation(str, svg, callback) {
	    ...
	    letter_1.buildAwesomeLetterAnimation(str, $svg, null, callback);
	}
	awesomeLetterAnimation['_testHello'] = function () {
	    var $svg = d3
	        ...
	        .style('width', 1000)
	        .style('height', 800);
	    letter_1.buildAwesomeLetterAnimation('hello world', $svg, { x: 200, y: 200 });
...
/***/ function(module, exports, __webpack_require__) {
	...
	"use strict";
	var letterConfigs_1 = __webpack_require__(4);
	var anime = __webpack_require__(2);
	var d3 = __webpack_require__(1);
	function _appendFillPath(letterCfg, $svg, offset) {
	    if (offset === void 0) { offset = { x: 0, y: 0 }; }
	    var offsetX = offset.x;
	    var offsetY = offset.y;
	    var pathd = letterCfg.path.map(function (_a, idx) {
	        var x = _a[0], y = _a[1];
	        var cmd = idx === 0 ? 'M' : 'L';
	...
	function _appendLinePath(letterCfg, $svg, offset) {
	    if (offset === void 0) { offset = { x: 0, y: 0 }; }
	    var offsetX = offset.x;
	    var offsetY = offset.y;
	    var path = letterCfg.path;
	    var invisibleIdxMap = !letterCfg.invisibleLines ? {} : letterCfg.invisibleLines.reduce(function (result, invisibleIdx) {
	        result[invisibleIdx] = true;
	        return result;
	    }, {});
	    var _a = path[0], prevX = _a[0], prevY = _a[1];
	    prevX += offsetX;
	    prevY += offsetY;
	    for (var i = 1, len = path.length; i < len; i++) {
	        var _b = path[i], currX = _b[0], currY = _b[1];
	        currX += offsetX;
	        currY += offsetY;
	        var pathNode = $svg.append('path')
	...
	function animateLetters(callback) {
	    if (callback === void 0) { callback = null; }
	    function getTranslate(linePathNode) {
	        function getLinePoints(linePathNode) {
	            var getPoint = function (pointStr) {
	                var _a = pointStr.split(','), x = _a[0], y = _a[1];
	        ...
	        var TRANSLATE_LENGTH = 1200;
	        var _a = getLinePoints(linePathNode), start = _a.start, end = _a.end;
	        if (start.x === end.x) {
	            // vertical
	            return {
	                translateX: 0,
	            ...
	            var translateY = Math.round(Math.sin(Math.atan(ratio)) * TRANSLATE_LENGTH);
	            return {
	    ...
	    anime({
	        ...
	        translateX: function (target) { return getTranslate(target).translateX; },
	        translateY: function (target) { return getTranslate(target).translateY; },
	        scale: 4,
	        opacity: {
	            value: [1, 0],
	            duration: 100,
	        ...
	        stroke: '#fff',
	        delay: function (target, index) { return index * 25; },
	        duration: 500,
	...
	exports.animateLetters = animateLetters;
	function getLetterBoundary(word) {
	    var path = letterConfigs_1.default[word].path;
	    var _a = path[0], minX = _a[0], minY = _a[1];
	    var maxX = minX, maxY = minY;
	    for (var i = 1, len = path.length; i < len; i++) {
	        var _b = path[i], x = _b[0], y = _b[1];
	        if (x < minX) {
	            minX = x;
	        }
	        else if (x > maxX) {
	...
	exports.getLetterBoundary = getLetterBoundary;
	function buildAwesomeLetterAnimation(str, $svg, offset, callback) {
	...
	exports.buildAwesomeLetterAnimation = buildAwesomeLetterAnimation;
